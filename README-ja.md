...
ネストのルール:
- `state(name)` は**1段降下**する — 対応する `end()` で昇格しなければならない。`build()` はネストが釣り合わない（`end()` 忘れ）定義を拒否し、root レベルでの `end()`（閉じるべき `state()` が無い）も拒否する。
- `initial(name)` と `terminal(name)` は**降下しない** — 現在の親の下に葉の子を登録し、そのまま親にとどまる。したがって直後に `end()` は**不要**。
- `onEntry` / `onExit` は**直前に宣言した状態**（`state`/`initial`/`terminal`/`root` のうち最後に呼んだもの）に付く。`end()` の後は昇格した親が対象。
- 合成状態の `initial()` は**正確に1つ**。同じ合成内で2つ目の `initial()` は `build()` で拒否される。
